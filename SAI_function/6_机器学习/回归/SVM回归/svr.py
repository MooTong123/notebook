...
def execute(conn, inputs, params, outputs, reportFileName):
    ...
    model = SVR(kernel=params['kernel'], degree=int(params['degree']), gamma=params['gamma'],
                coef0=float(params['coef0']), C=float(params['C']), tol=float(params['tol']),
                epsilon=float(params['epsilon']), shrinking=bool(params['shrinking']),
                cache_size=int(params['cache_size']), max_iter=int(params['max_iter']))
