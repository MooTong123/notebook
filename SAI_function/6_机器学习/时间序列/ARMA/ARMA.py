...
def execute(conn, inputs, params, outputs, reportFileName):
    ...
    训练和拟合模型
    '''
    model = ARIMA(data, order=(int(params['p']), 0, int(params['q']))).fit()
    ...
    md_info['值'] = ['%.4f' % mean_squared_error(data, ts_fit),
                    '%.4f' % np.sqrt(mean_squared_error(data, ts_fit)),
                    '%.4f' % mean_absolute_error(data, ts_fit),
                    '%.4f' % explained_variance_score(data, ts_fit),
                    '%.4f' % r2_score(data, ts_fit)]
    ...
    '''
    report.h3('模型拟合情况')
    report.p('建立ARMA模型,设置p,q为：{},{}，得到的预测值与实际值对比图如下图所示。'.format(params['p']),params['q'])
    plt.figure(figsize=(6.0, 4.0))
